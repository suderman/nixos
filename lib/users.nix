{flake, ...}: let
  ...
  rotationAuthorizedKeys = authorizedKeys:
    authorizedKeys
    // {
      keyFiles = identityRotation.keyFiles (authorizedKeys.keyFiles or []);
    };
in
    ...
    in
      ...
      // (
        ...
        then rec {
          ...
          linger = false;
          openssh = {
            authorizedKeys = rotationAuthorizedKeys (user.openssh.authorizedKeys or {});
