...
{ config, lib, pkgs, this, ... }: let
  ...
  labels = args: ( let
    inherit (builtins) elemAt head isList isString length toBool toString;
    ...
    fromList = args: let 
      ...
      # Determine hostName from first argument
      hostName = mkHostName (head args);

      # Replace all dots with underscores
      name = replaceStrings ["."] ["_"] (head args);

      # Optional port and scheme arguments
      port = if (length args > 1) then toString (elemAt args 1) else "";
      scheme = if (length args > 2) then toString (elemAt args 2) else "";

      # If the hostName DOES end with this system's hostName, assume private
      # If the hostName DOESN'T end with this system's hostName, assume public
      public = if hasSuffix ".${this.hostName}" hostName then false else true;
    ...
    # and include "local" whitelist middleware
    ] else [
      "--label=traefik.http.routers.${name}.tls=true"
      "--label=traefik.http.routers.${name}.middlewares=local@file" 

    # Optional port and scheme
    ]) ++ ( if port == "" then [] else [
