{ config, lib, flake, ... }: let
  ...
  mkRouter = name: url: let
    ...
    fromAttrs = { hostName ? mkHostName name, tls ? null, public ? null, middlewares ? [], ... }: let
      ...
      external' = if elem hostName ([ cfg.hostName ] ++ cfg.extraInternalHostNames) || hasSuffix ".${cfg.hostName}" hostName || hasSuffix ".${cfg.domainName}" hostName then false else true;  
  ...
  mkAlias = name: args: ( let
    ...
    defaultPublic = hostName: if hasSuffix ".${cfg.domainName}" hostName then false else true;
