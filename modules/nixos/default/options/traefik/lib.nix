{
  ...
  cfg =
    ...
    // {
      ...
      inherit (flake.networking) domainName;
    };
  ...
  mkHostName = arg: let
    ...
    fromName = name:
      ...
      else "${name}.${cfg.hostName}"; # else, prepend "name" to system's hostname
  in
    if hasInfix "://" arg
    then fromUrl arg
    else fromName arg;
  ...
  mkHostNames = {
    ...
    externalRules' = rule: (hasInfix "Host(`" rule) && (! hasInfix ".${cfg.hostName}`" rule);
    internalRules' = rule: (hasInfix "Host(`" rule) && (hasInfix ".${cfg.hostName}`" rule);
    ...
    localHostName =
      if public != true && external != true
      then ["local"]
      else [];
  in
    localHostName ++ routerHostNames ++ labelHostNames;
  ...
  mkRouter = name: url: let
    ...
    fromAttrs = {
      ...
      external' =
        if elem hostName ([cfg.hostName] ++ cfg.extraInternalHostNames) || hasSuffix ".${cfg.hostName}" hostName || hasSuffix ".${cfg.domainName}" hostName
        then false
        else true;
  ...
  mkLabels = args: (
    let
      ...
      fromList = args:
        fromAttrs {
          ...
          port =
            if (length args > 1)
            then toString (elemAt args 1)
            else null;
          scheme =
            if (length args > 2)
            then toString (elemAt args 2)
            else null;
        };
      ...
      fromAttrs = {
        ...
        tls' =
          ...
          then [
            ...
            # Internal hostNames will use certificates generated by the custom CA
    ...
    in
      if isString args
      then fromString args
      else if isList args
      then fromList args
      else if isAttrs args
      then fromAttrs args
      else []
  );
  ...
  mkAlias = name: args: (
    let
      ...
      defaultPublic = hostName:
        if hasSuffix ".${cfg.domainName}" hostName
        then false
        else true;
      ...
      in
        fromAttrs {inherit hostName public;};
