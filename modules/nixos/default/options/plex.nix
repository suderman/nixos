# services.plex.enable = true;
{
  ...
  inherit (flake.lib) extraGroups sudoers users;
in {
  ...
  config = mkIf cfg.enable {
    ...
    # Add sudoers to the plex group
    users.users = extraGroups (sudoers users) ["plex"];

    # https://www.plex.tv/claim/
    # sudo plex-claim-server claim-xxxxxxxxxxxxxxxxxxxx
    environment.systemPackages = [
      (pkgs.writeShellScriptBin "plex-claim-server" (builtins.readFile
        # bash
        ''
