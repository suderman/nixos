{
  ...
  inputs = {
    ...
    # nvf.url = "github:suderman/nixos/blueprint";
  ...
  outputs = {self, ...}: let
    ...
    unstableInputs = mapAttrs (k: v:
      if hasAttr "${k}-unstable" inputs
      then inputs."${k}-unstable"
      else v)
    inputs;
    ...
    mkPkgs = this:
      import this.inputs.nixpkgs rec {
    ...
    mkConfiguration = this:
      this.inputs.nixpkgs.lib.nixosSystem rec {
