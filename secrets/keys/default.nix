# 1def5a60ba13493b2d0d3a5b6483de04
# Do not modify this file!  It was generated by ‘nixos rekey’ 
# and may be overwritten by future invocations. 
# Please add public keys to /etc/nixos/secrets/keys/*.pub 
rec {

  users.blink = "ssh-rsa ";
  users.me = "ssh-rsa ";
  users.all = [ users.blink users.me ];

  systems.cog = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPm+/Hq+sZM78OZnWY8DT/7O3RGXb0j1+mYElwquD4LJ";
  systems.lux = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIAfPrHGL3ZkParCHImTtMnxphq3O0UF/L25RDRz28Xeo";
  systems.nimbus = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIJRdhr2F1Z4YpQZwNet6FGoqgPU43mbj7rP8pd+RLehf";
  systems.sol = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAINSK0ArOfOAsvPf3RWTilZqHTUrmQK9EABZksldINNyV";
  systems.test-1678381076 = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIAfPrHGL3ZkParCHImTtMnxphq3O0UF/L25RDRz28Xeo";
  systems.test-1678381279 = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIAfPrHGL3ZkParCHImTtMnxphq3O0UF/L25RDRz28Xeo";
  systems.test = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIAfPrHGL3ZkParCHImTtMnxphq3O0UF/L25RDRz28Xeo";
  systems.all = [ systems.cog systems.lux systems.nimbus systems.sol systems.test-1678381076 systems.test-1678381279 systems.test ];

  all = users.all ++ systems.all;

}
