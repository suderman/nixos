  ...
  lib = {
    ...
    ls = x: (
      let
        ...
        fromAttrs = {
          ...
          dirsExcept ? ["user"],
          filesExcept ? ["flake.nix" "default.nix" "configuration.nix" "this.nix"],
          asPath ? true,
        }:
          unique
          (
    ...
    mkUsers = hostName: fn:
      builtins.listToAttrs (
        ...
        (ls {
          path = ./configurations/systems/${hostName}/users;
          asPath = true;
          dirsExcept = [];
        })
      );
    ...
    nameFromPath = path: let
      ...
      inherit (inputs.nixpkgs.lib) removeSuffix;
    in
      baseNameOf (removeSuffix ".nix" (removeSuffix "/this.nix" (removeSuffix "/configuration.nix" (removeSuffix "/default.nix" (removeSuffix "/user/default.nix" (toString path))))));

    # Importable configuration profiles
    mkProfiles = this: let
      profilesFromPath = dirName:
        builtins.listToAttrs (
          map
          (path: {
            name = nameFromPath path;
          ...
          (ls {
            ...
            asPath = true;
            dirsWith = ["${dirName}/default.nix"];
          })
        );
    in {
      root = profilesFromPath "/";
      user = profilesFromPath "/user";
    };
    ...
    mkModules = this: let
      ...
      in {
        ...
        home = [nix-index-database.homeModules.nix-index config];
      };
    ...
    in
      ...
      mkUsers hostName (
        userPath: let
          userName = nameFromPath userPath;
        in
          ls {
            path = ./modules;
            dirsWith = ["user/default.nix"];
          }
          ++ # home-manager modules
          ls ./configurations/users/all/default.nix
          ++ # shared home-manager configuration for all users
          ls ./configurations/users/${userName}.nix
          ++ # shared home-manager configuration for one user
          ls ./configurations/users/${userName}/default.nix
          ++ ls ./configurations/systems/${hostName}/users/${userName}.nix
          ++ # specific home-manager configuration for one user
      ...
      // {
        root =
          ...
          ++ # nixos modules
          ls ./configurations/systems/all/configuration.nix
          ++ # shared nixos configuration for all systems
          ls ./configurations/systems/${hostName}/configuration.nix
          ++ # specific nixos configuration for one system
          [./secrets] ++ nix-cache ++ nix-index.nixos
          # secrets, keys, cache and index
          ;
      };
  };
in {
