...
def create_ca_certificate(private_key):
    ...
    name = x509.Name(
        [
            ...
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Suderman CA"),
            x509.NameAttribute(NameOID.COMMON_NAME, "Suderman CA"),
        ]
    )
...
def create_server_certificate(subject_key, ca_cert, ca_key, common_name):
    ...
    subject_name = x509.Name(
        [
            x509.NameAttribute(NameOID.COUNTRY_NAME, "CA"),
            x509.NameAttribute(NameOID.STATE_OR_PROVINCE_NAME, "Alberta"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Suderman Services"),
    ...
    builder = builder.add_extension(
        x509.SubjectAlternativeName(
            [x509.DNSName(common_name), x509.DNSName(f"*.{common_name}")]
        ),
        critical=False,
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create a certificate from an Ed25519 private key"
    )
    ...
    parser.add_argument(
        "--name", help="Common name for the server certificate (formerly --common-name)"
    )
    ...
    try:
        ...
        if args.cacert and args.cakey and args.name:
            ...
            cert_pem = create_server_certificate(
                private_key, ca_cert, ca_key, args.name
            )
        else:
