...
for host in $(eza -D hosts); do
  ...
  if has hosts/$host/ssh_host_ed25519_key.pub; then
    hint "Public host key exists: ./hosts/$host/ssh_host_ed25519_key.pub"
  else
    cat secrets/hex.age \
      | rage -di /tmp/id_age \
      | derive hex "$host" \
  ...
  if has hosts/$host/ssh_host_ed25519_key.age; then
    hint "Secret host key exists: ./hosts/$host/ssh_host_ed25519_key.age"
  else
    cat secrets/hex.age \
      | rage -di /tmp/id_age \
      | derive hex "$host" \
    ...
    info "Secret host key staged: ./hosts/$host/ssh_host_ed25519_key.age"
  fi
...
for user in $(eza -D users); do
  ...
  if has users/$user/id_ed25519.pub; then
    hint "Public user key exists: ./users/$user/id_ed25519.pub"
  else
    cat secrets/hex.age \
      | rage -di /tmp/id_age \
      | derive hex "$user" \
  ...
  if has users/$user/id_ed25519.age; then
    hint "Secret user key exists: ./users/$user/id_ed25519.age"
  else
    cat secrets/hex.age \
      | rage -di /tmp/id_age \
      | derive hex "$user" \
    ...
    info "Secret user key staged: ./users/$user/id_ed25519.age"
  fi
