...
- `secrets/rotation/state.json` is an inert, non-secret target ledger. Validate changes with `nix build 'path:.#checks.x86_64-linux.identity-rotation' -L`; test fixture roots must never be used outside that check.
